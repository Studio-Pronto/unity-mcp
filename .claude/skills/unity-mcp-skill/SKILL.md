...
| **ScriptableObjects** | `manage_scriptable_object` | Create/modify SO assets with serialized property patching, GUID & path object references, arrays, dry-run. **Requires `scripting_ext` group** — activate via `manage_tools(action="activate", group="scripting_ext")`. |
